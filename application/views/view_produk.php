  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
          ...
          <div class="row">
            ...
            <div class="col-md-6">
              <div class="form-group">
                ...
                <input type="text" id="barcode" name="barcode" autocomplete="off" class="form-control input-sm" placeholder="Barcode">
              </div>
            </div>
          </div>
      ...
      <div class="modal-footer">
        ...
        <button type="button" class="btn btn-primary" id="btnSimpan" data-loading-text="<i class='fa fa-circle-o-notch fa-spin'></i> Processing ">Simpan
          Data</button>
      </div>
    </div>
  </div>
...
<script>
      ...
      success: function(response) {
        ...
        for (i = 0; i < response.length; i++) {
          ...
          html = html + '<tr>' +
            ...
            response[i].id_produk + '" class="btn btn-danger btn-xs btn_hapus"><i class="fa fa-trash"></i> Hapus</button></span>' + '</td>' +
            '</tr>'
        }
        $("#tbl_data").html(html);
        $('#mydata').DataTable();
  ...
  //Memanggil Modal Produk
  $(document).on("click", "#btnTambah", function(e) {
    e.preventDefault();
    bEdit = false;
    $('#form_add')[0].reset(); //reset form on modals
    $('.form-group').removeClass('has-error'); //clear class error
  ...
  $("#tbl_data").on('click', '.btn_edit', function() {
    ...
    $.ajax({
      ...
      success: function(response) {
        ...
        $('.form-group').removeClass('has-error'); //clear class error
        $('.help-block').empty(); //clear error string
        $('.modal-title').text('Edit Barang'); //Set Title to Bootstrap
